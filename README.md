...
> **💡 初回起動時のモデルダウンロードについて**
> - 初回の変換リクエスト時に、Docling 解析モデル（OCR、レイアウト解析等）が Hugging Face Hub 等から自動的にダウンロードされます（初回のみ数百MB〜約1GBの通信が発生します）。
> - ダウンロードされたモデルは Docker ボリューム（`docling_data` 内の `/app/data/models`）に永続化されるため、コンテナを再起動・再ビルドしても再ダウンロードは発生しません。
> - 詳細は [Deployment Guide (DEPLOYMENT.md)](docs/DEPLOYMENT.md) を参照してください。
