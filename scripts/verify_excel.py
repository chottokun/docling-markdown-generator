...
def main():
    ...
    xlsx_path = (
        project_root / "tests" / "data" / "real_world" / "gijutsu_matrix_20260214.xlsx"
    )
    ...
    try:
        ...
        if result_path and result_path.exists():
            ...
            if "<table>" in content:
                logger.info(
                    "High-fidelity HTML table structures were found in the output!"
                )
            else:
                logger.info(
                    "No HTML tables found. Standard markdown table or plain text might be used."
                )

        else:
            logger.error(
                "Conversion failed. process_pdf returned None or file does not exist."
            )
            sys.exit(1)
