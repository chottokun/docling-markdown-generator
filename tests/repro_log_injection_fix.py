...
docling_core.transforms.serializer.markdown = mock_module(
...
from docling_lib.converter import process_pdf  # noqa: E402
...
def test_workflow_error_log_injection_v2():
    ...
    try:
        with (
            patch("docling_lib.converter._validate_input_path", return_value=True),
            patch("docling_lib.converter._validate_output_security", return_value=True),
            patch(
                "docling_lib.converter._get_or_create_converter",
                side_effect=Exception(bad_message),
            ),
        ):
        ...
        for record in log_records:
            ...
            if "Workflow Error:" in msg:
                ...
                assert "\n" not in msg, (
                    f"Vulnerability present: newline found in log message: {repr(msg)}"
                )
                assert "\r" not in msg
