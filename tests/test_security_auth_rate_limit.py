...
def test_auth_enabled():
    ...
    with patch("docling_lib.server.API_KEY", "test-api-key"):
        ...
        # Wrong API Key
        response = client.post("/convert/", files=files, headers={"X-API-Key": "wrong-key"})
        assert response.status_code == 401
        ...
        with patch("docling_lib.server._validate_extension", return_value=".pdf"):
             with patch("docling_lib.server._save_upload_temp", return_value=Path("dummy_path")):
                 with patch("docling_lib.server._create_output_dir", return_value=("id", Path("out"))):
