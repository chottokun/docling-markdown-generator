...
def test_auto_block_newline_complexity():
    ...
    long_text = (
        "x = \\frac{-b \\pm \\sqrt{b^2 - 4ac}}{2a} + \\int_{a}^{b} f(t) dt + g(x)"
    )
    complex_item_long = FormulaItem(
        self_ref="#/formula/2", text=long_text, orig=long_text
    )
    res_complex_long = serializer.serialize(
        item=complex_item_long,
        doc_serializer=doc_serializer,
        doc=doc,
        is_inline_scope=False,
    )
    ...
    complex_item_multiline = FormulaItem(
        self_ref="#/formula/3", text=multiline_text, orig=multiline_text
    )
    res_complex_multiline = serializer.serialize(
        item=complex_item_multiline,
        doc_serializer=doc_serializer,
        doc=doc,
        is_inline_scope=False,
    )
...
def test_pdf_converter_math_options_propagation():
    ...
    with (
        ...
        converter._serialize_to_markdown(mock_doc, table_format, options=custom_options)
...
def test_fastapi_server_math_form_fields():
    ...
    with patch("docling_lib.server.process_pdf") as mock_process_pdf:
        ...
        response = client.post(
            ...
            },  # The global_api_key_override fixture handles this
