...
def test_rate_limit_respects_x_forwarded_for():
    ...
    with patch("docling_lib.server.RATE_LIMIT_REQUESTS", 1):
        with patch("docling_lib.server.RATE_LIMIT_WINDOW", 60):
            with patch("docling_lib.server.TRUST_PROXIES", True):
...
def test_rate_limit_ignores_headers_when_trust_proxies_false():
    # Reset rate limit data
    docling_lib.server._rate_limit_data.clear()

    with patch("docling_lib.server.RATE_LIMIT_REQUESTS", 1):
        with patch("docling_lib.server.RATE_LIMIT_WINDOW", 60):
            with patch("docling_lib.server.TRUST_PROXIES", False):
                with patch("docling_lib.server.API_KEY", None):
                    client = TestClient(app)

                    # Request 1 with spoofed headers
                    response1 = client.get("/download/id1/file1.md", headers={"X-Forwarded-For": "1.1.1.1", "X-Real-IP": "2.2.2.2"})
                    assert response1.status_code != 429

                    # Request 2 with DIFFERENT spoofed headers but same client host (testclient defaults to 127.0.0.1)
                    # Since headers are ignored, both should count as 127.0.0.1 and thus be rate limited!
                    response2 = client.get("/download/id2/file2.md", headers={"X-Forwarded-For": "3.3.3.3", "X-Real-IP": "4.4.4.4"})
                    assert response2.status_code == 429
